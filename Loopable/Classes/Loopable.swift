    ...
    var toDictionary: [String: Any]? {
        ...
        if let superMirror = superclassMirror {
            if let superResult = superMirror.toDictionary {
                result.merge(superResult) { (current, new) -> Any in
...
extension Loopable {
    ...
    public func json(options: [JSONOption] = [.onlyCompatible]) -> [String: Any] {
        ...
        for (property, value) in props {
            if let tmp = value as? Dictionary<String, Any> {
                var dict = [String: Any]()
                for (k, v) in tmp {
                ...
                json[property] = dict
            } else if let tmp = value as? Array<Any> {
                var buf = [Any]()
                for item in tmp {
...
public extension Array where Element == Any {
    ...
    func dropNull() -> [Any] {
        return filter {
            if case Optional<Any>.none = $0 { return false }
            else { return true }
        }
    }
}
...
public extension Dictionary where Key == String, Value == Any {
    ...
    func dropNull() -> [String: Any] {
        return filter {
            if case Optional<Any>.none = $0.value { return false }
            else { return true }
        }
    }
}
